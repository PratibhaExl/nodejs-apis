...
document.querySelectorAll( "h1,h2,p,span.cs-section-title,li,span.article-list-item-span-text.analytics-cta-label-child-class,div.author-desc").forEach(h => {
  if (!isVis(h)) return;
  ...
  if (h.matches("h1,h2,span.cs-section-title")) {
    ...
    // ✅ Ensure first paragraph after heading is always captured
...
//     arr.some(s => s !== sec && s.content.some(c => c.includes(sec.heading)))
//   ) {
//     return false;
//   }
